package com.greensock.plugins
{
   ...
   public class MotionBlurPlugin extends TweenPlugin
   {
      ...
      override public function onInitTween(param1:Object, param2:*, param3:TweenLite) : Boolean
      {
         ...
         {
            x = Number(Number(this._tween.vars.x) || this._target.x);
            y = Number(Number(this._tween.vars.y) || this._target.y);
         ...
         {
            this._sprite = new value.containerClass();
         }
         else if(_isFlex)
         {
            this._sprite = new (getDefinitionByName("mx.core.UIComponent"))();
         }
         else
         {
         ...
         if(Boolean(this._target.mask))
         {
            this._mask = this._target.mask;
         }
         if(Boolean(this._target.parent) && Boolean(this._target.parent.mask))
         {
      ...
      private function disable() : void
      {
         ...
         if(Boolean(this._sprite.parent))
         {
      ...
      override public function set changeFactor(param1:Number) : void
      {
         ...
         if(this._sprite.parent != this._target.parent && Boolean(this._target.parent))
         {
         ...
         if(!this._fastMode || !this._cached)
         {
            ...
            {
               this._blur.blurX = _loc12_.width >= 2870 ? 0 : (2870 - _loc12_.width) * 0.5;
            }
