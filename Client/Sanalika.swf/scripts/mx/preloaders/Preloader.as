package mx.preloaders
{
   ...
   public class Preloader extends Sprite
   {
      ...
      private function getByteValues() : Object
      {
         ...
         var _loc4_:int = Boolean(this.rslListLoader) ? this.rslListLoader.getItemCount() : 0;
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc2_ += this.rslListLoader.getItem(_loc5_).loaded;
            _loc6_ = int(this.rslListLoader.getItem(_loc5_).total);
            _loc3_ += _loc6_;
            _loc5_++;
         }
         return {
