package com.oyunstudyosu.engine.core
{
   ...
   public class BotEntry extends MapEntry implements ISpeechable
   {
      ...
      override public function init(param1:IsoScene) : void
      {
         super.init(param1);
      ...
      private function onLoaded(param1:IAssetRequest) : void
      {
         ...
         _loc3_ = 0;
         while(_loc3_ < clip.numChildren)
         {
            if(clip.getChildAt(_loc3_).name.indexOf("questIcon") == 0)
            {
               clip.getChildAt(_loc3_).visible = true;
               TweenMax.to(clip.getChildAt(_loc3_),0.3,{
                  "glowFilter":{
                     "color":16777215,
                     "alpha":1,
                     "blurX":15,
               ...
               clip.getChildAt(_loc3_).y = -clip.getChildAt(0).height - 10;
            }
            _loc3_++;
         }
         param1.dispose();
      ...
      private function speechBalloonDisappeared(param1:Event) : void
      {
         ...
         if(_loc2_ && Sanalika.instance.engine.scene.speechBalloons.indexOf(_loc2_) > -1)
         {
            _loc2_.removeEventListener("disappeared",speechBalloonDisappeared);
            Sanalika.instance.engine.scene.speechBalloons.splice(Sanalika.instance.engine.scene.speechBalloons.indexOf(_loc2_),1);
