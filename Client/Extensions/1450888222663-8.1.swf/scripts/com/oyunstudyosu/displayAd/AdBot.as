package com.oyunstudyosu.displayAd
{
   ...
   public class AdBot
   {
      ...
      public function execute(param1:GameEvent) : void
      {
         ...
         TweenMax.delayedCall(2,ekle);
      }
      
      public function ekle() : void
      {
         npc = new Character();
         npc.init($("TutorialCharName"),Sanalika.instance.engine.scene as IsoScene,0,botGender,activeClothes);
         npc.reLocate(position.x,position.y,direction);
         npc.avatarName = $("TutorialCharName");
         npc.avatarNick = $("TutorialCharName");
         npc.isNPC = true;
         npc.useHandItem("dL4WAYyX");
         Sanalika.instance.engine.scene.characterList.push(npc);
         if(data)
         {
      ...
      protected function adClicked(param1:MouseEvent) : void
      {
         ...
         npc.removeHandItem();
         TweenMax.delayedCall(10,dispose);
      }
